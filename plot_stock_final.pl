...
$sql .= " union select timestamp, close from stocksdailyaddon where symbol=(?)";
$sql .= " and timestamp >= $from" if $from;
$sql .=" and timestamp <= $to" if $to;
$sql .=" order by timestamp";
my @rows = ExecStockSQL("2D",$sql,@symbols);
...
#
  ...
  print GNUPLOT "set xlabel 'Timestamp'\n";
  print GNUPLOT "set ylabel 'Close'\n";
